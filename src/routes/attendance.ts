...
import {
  getCurrentTeacher,
  getCurrentReportViewer,
} from "../services/currentTeacher";
...
export const attendanceRouter = new Elysia({
  ...
  .get("/semester-summary", async ({ query, headers, set }) => {
    try {
      const current = await getCurrentReportViewer(headers);
      ...
      // ==========================
      // Validasi Hak Akses
      // Kepala sekolah boleh lihat semua kelas, jadi validasi wali kelas
      // hanya berlaku untuk role guru.
      // ==========================

      if (current.role === "guru" && teacher.teacher_type === "kelas") {
  ...
  `,
      ...
      // ==========================
      // Validasi guru kelas
      // (dilewati untuk kepala sekolah — boleh akses semua kelas)
      // ==========================
  ...
  .get("/export/semester/xlsx", async ({ query, headers, set }) => {
    try {
      ...
      // ==========================
      // DATA ABSENSI
      // Untuk guru: difilter berdasarkan teacher_id guru yang login
      // (supaya rekap guru kelas dan guru mapel tidak tercampur).
      // Untuk kepala sekolah: tanpa filter teacher_id, mencakup semua
      // guru yang mengajar di kelas & semester tersebut.
      // ==========================

      const detailsFilterTeacher = current.role === "guru";

      const [details]: any = await db.query(
        `
      SELECT
        ad.student_id,
        DATE_FORMAT(ats.tanggal, '%Y-%m-%d') AS tanggal,
        ad.status
      ...
      WHERE ats.class_id = ?
        ${detailsFilterTeacher ? "AND ats.teacher_id = ?" : ""}
        AND ats.academic_period_id = ?
        AND ats.status = 'selesai'
      `,
        detailsFilterTeacher
      ...
      // ==========================
      // GROUP PER BULAN
      // Gunakan string tanggal langsung dari DB (format YYYY-MM-DD)
      // untuk menghindari timezone shift saat new Date() menginterpretasikan
      // string tanggal tanpa waktu sebagai UTC midnight.
      // ==========================

      const monthMap = new Map<string, any[]>();

      for (const row of details) {
        // row.tanggal dari MySQL DATE column: bisa berupa Date object atau string
        // Ambil 7 karakter pertama (YYYY-MM) langsung tanpa konversi timezone
        const tanggalStr =
          row.tanggal instanceof Date
            ? `${row.tanggal.getFullYear()}-${String(row.tanggal.getMonth() + 1).padStart(2, "0")}`
            : String(row.tanggal).slice(0, 7);

        if (!monthMap.has(tanggalStr)) {
          monthMap.set(tanggalStr, []);
        }

        monthMap.get(tanggalStr)?.push(row);
      }

      // ==========================
      // SHEET PER BULAN
      // Jika tidak ada data sama sekali, buat 1 sheet kosong agar file
      // xlsx tetap valid dan tidak rusak saat dibuka Excel.
      // ==========================

      if (monthMap.size === 0) {
        const emptySheet = workbook.addWorksheet("Tidak Ada Data");
        emptySheet.addRow([
          "Belum ada data absensi yang selesai untuk kelas dan semester ini.",
        ]);
      }
      ...
      for (const [monthKey, records] of monthMap) {
        ...
        // ==========================
        // MAP ABSENSI
        // Gunakan string tanggal langsung dari DB untuk menghindari
        // timezone shift (new Date("YYYY-MM-DD") = UTC midnight).
        // ==========================
        ...
        for (const item of records) {
          const tanggalKey =
            item.tanggal instanceof Date
              ? `${item.tanggal.getFullYear()}-${String(item.tanggal.getMonth() + 1).padStart(2, "0")}-${String(item.tanggal.getDate()).padStart(2, "0")}`
              : String(item.tanggal).slice(0, 10);

          const key = `${item.student_id}-${tanggalKey}`;
  ...
  .get("/class/:id/semester-recap", async ({ params, query, headers, set }) => {
    try {
      ...
      // ==================================
      // VALIDASI AKSES GURU KELAS
      // (kepala sekolah dilewatkan — boleh akses semua kelas)
      // ==================================
      ...
      if (current.role === "guru" && teacher.teacher_type === "mapel") {
        const [kelas]: any = await db.query(
      ...
      // SEMUA SESI ABSENSI
      // ======================
      // Untuk guru: difilter berdasarkan teacher_id guru yang login, supaya
      // guru mapel hanya melihat rekap mata pelajaran & kelas yang dia ampu
      // sendiri, tidak tercampur dengan sesi guru lain di kelas yang sama.
      // Untuk kepala sekolah: tanpa filter teacher_id, supaya rekap
      // mencakup SEMUA guru yang mengajar kelas & semester tersebut.

      const recapFilterTeacher = current.role === "guru";

      const [sessions]: any = await db.query(
        `SELECT id, tanggal, session_type, subject_id
         FROM attendance_sessions
         WHERE class_id = ?
           ${recapFilterTeacher ? "AND teacher_id = ?" : ""}
      ...
      const [details]: any = await db.query(
        `SELECT ad.student_id, ad.session_id, ad.status
         FROM attendance_details ad
         JOIN attendance_sessions s ON s.id = ad.session_id
         WHERE s.class_id = ?
           ${recapFilterTeacher ? "AND s.teacher_id = ?" : ""}
  ...
  .get("/statistics", async ({ headers, query, set }) => {
    try {
      ...
      // HARI INI
      // ======================
      // Untuk guru: hanya sesi milik guru yang login.
      // Untuk kepala sekolah: seluruh sesi sekolah (semua guru).

      const statsFilterTeacher = current.role === "guru";

      const today = new Date().toISOString().slice(0, 10);

      const [todayRows]: any = await db.query(
        `SELECT COUNT(DISTINCT s.id) AS sessions, SUM(ad.status='hadir') AS hadir, SUM(ad.status='izin') AS izin, SUM(ad.status='sakit') AS sakit, SUM(ad.status='alpha') AS alpha
         FROM attendance_sessions s
         LEFT JOIN attendance_details ad ON ad.session_id = s.id
         WHERE ${statsFilterTeacher ? "s.teacher_id = ? AND" : ""} s.tanggal = ? AND s.status = 'selesai'`,
        statsFilterTeacher ? [teacher.id, today] : [today],
      );
      ...
      const [semesterRows]: any = await db.query(
        `SELECT COUNT(DISTINCT s.id) AS sessions, COUNT(ad.id) AS total_records, SUM(ad.status='hadir') AS hadir, SUM(ad.status='izin') AS izin, SUM(ad.status='sakit') AS sakit, SUM(ad.status='alpha') AS alpha
         FROM attendance_sessions s
         LEFT JOIN attendance_details ad ON ad.session_id = s.id
         WHERE ${statsFilterTeacher ? "s.teacher_id = ? AND" : ""} s.academic_period_id = ? AND s.status = 'selesai'`,
        statsFilterTeacher ? [teacher.id, period.id] : [period.id],
      );
